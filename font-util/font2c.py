...
class font2c():
    ...
    def preview(self):
        ...
        height = int(
            ((len(self.conf.text) / DISPLAY_COLUMN_CHAR) + 1) * (fnt.getsize('X')[1] + DISPLAY_ROW_CLEARANCE)) * 2
    ...
    def encoding_method_raw_origin(self, img, x0, y0, x1, y1):
        ...
        if (count != 0):
            ...
            print(" ".join(format(byte, '02x')))
        print(" ".join(format(x, '02x') for x in bs))
        return bs

    def encoding_method_raw(self, img, x0, y0, x1, y1):
        # Scan from left to right, down to bottom sequentially
        print("Lets debug encoding raw!")
        print("Size: x from {} to {}, y from {} to {}".format(x0, x1, y0, y1))
        outarray = numpy.zeros((y1 - y0,), dtype=numpy.uint16)
        word_count = 0
        for y in range(y0, y1):
            count = 0
            word = 0
            for x in range(x0, x1):
                pixel = self._img_push_pixel_to_steam(img, (x, y))
                if (pixel):
                    print("x={}, y={}, val={}".format(x, y, pixel))
                else:
                    print("x={}, y={}, val={}".format(x, y, pixel))
                word |= (pixel << count)
                count += self.conf.bpp
            outarray[word_count] = word
            print("Word appended: {0:016b}".format(word))
            word_count += 1
        print(" ".join(format(x, '02x') for x in outarray))
        print("\n".join("{0:016b}".format(x) for x in outarray))
        return outarray
    ...
    def export(self):
        ...
        with open(self.conf.export_dir + '/' + self.conf.c_filename + ".h", "w", encoding='utf8') as cfile:
            ...
            for template in template_list:
                ...
                # If encoding method is raw and fixed_width_length != None, bmplen can be pre-estimated
                ...
                for c in self.conf.text:
                    ...
                    img.save(
                        self.conf.export_dir + '/img/' + font_name + ('_l' if c.islower() else '_') + alias_c + '.png')
                    ...
                    template_key['bmpdata'] = ',\n    '.join(
                        [', '.join(['0x{:04X}'.format((x)) for x in bmpsteam[y: y + self.ROWSIZE]]) for y in
                         range(0, len(bmpsteam), self.ROWSIZE)])
